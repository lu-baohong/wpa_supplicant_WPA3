...
skip_auth_type:
	...
	if (ret && retry_sae == 0 && (params->auth_alg & WPA_AUTH_ALG_SAE)) {
		retry_sae = 1;
		wpa_printf(MSG_DEBUG, "nl80211: Try SAE with SHARED");
		goto begin;
	}
...
#ifdef ANDROID
	wpa_printf(MSG_DEBUG, "nl80211: try private command");

	extlen = strlen("EXT_AUTH_STATUS ");

	wpa_hexdump(MSG_DEBUG, "nl80211: parameters dump",
		params, sizeof(struct external_auth));

	data = os_malloc(64 + PMKID_LEN + extlen);

	snprintf((char *)data, extlen, "EXT_AUTH_STATUS ");

	WPA_PUT_BE32((data + extlen), params->action);
	if (params->bssid)
		os_memcpy((data + extlen + 4), params->bssid, ETH_ALEN);

	if (params->ssid && params->ssid_len) {
		os_memcpy((data + extlen + 10), params->ssid, params->ssid_len);
		WPA_PUT_BE64((data + extlen + 42), params->ssid_len);
	}
	WPA_PUT_BE32((data + extlen + 58), params->key_mgmt_suite);
	WPA_PUT_BE16((data + extlen + 62), params->status);
	if (params->pmkid) {
		os_memcpy((data + extlen + 64), params->pmkid, PMKID_LEN);
	}

	wpa_hexdump(MSG_DEBUG, "nl80211: data dump",
		data, 64 + PMKID_LEN + extlen);

	ret = wpa_driver_nl80211_driver_cmd(priv, (char *)data,
		(char *)data, 64 + PMKID_LEN + extlen);
	os_free(data);
#else
