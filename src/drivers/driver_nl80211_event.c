		       ...
		       struct nlattr *wmm)
{
	...
	/* This is special AUTH FRAME for SAE trigger */
	if ((cmd == NL80211_CMD_FRAME) &&
		(RSN_SELECTOR_GET(data + 18) == RSN_AUTH_KEY_MGMT_SAE)) {
		uint32_t suite = WPA_GET_LE32(data + 18);
		uint16_t subtype = WPA_GET_LE16(data + 24);
		...
		if (!(msg = nl80211_drv_msg(drv, 0, NL80211_CMD_EXTERNAL_AUTH)) ||
			nla_put_u32(msg, NL80211_ATTR_AKM_SUITES, suite) ||
			nla_put_u32(msg, NL80211_ATTR_EXTERNAL_AUTH_ACTION, NL80211_EXTERNAL_AUTH_START) ||
			nla_put(msg, NL80211_ATTR_BSSID, ETH_ALEN, (data + 4)) ||
			nla_put(msg, NL80211_ATTR_SSID, ssid_len, ssid_ptr)) {
			nlmsg_free(msg);
			wpa_printf(MSG_INFO, "nl80211: MLME event frame"
				" change to EXTERNAL_AUTH fail");
			return;
		}
