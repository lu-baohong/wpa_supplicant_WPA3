...
#ifdef REALTEK_FEATURES
static int wpa_drv_driver_cmd_set_aek(struct wpa_supplicant *wpa_s, struct sta_info *sta)
{
#define SET_AEK_DATA_LEN (ETH_ALEN + 32)
#define CMD_BUF_LEN (7 + 1 + SET_AEK_DATA_LEN)
	char *cmd_str = "SET_AEK";
	char cmd[CMD_BUF_LEN];
	int ret;

	strcpy(cmd, cmd_str);
	memcpy(cmd + strlen(cmd_str) + 1, sta->addr, ETH_ALEN);
	memcpy(cmd + strlen(cmd_str) + 1 + ETH_ALEN, sta->aek, 32);

	ret = wpa_drv_driver_cmd(wpa_s, cmd_str, cmd, CMD_BUF_LEN);

	return ret;
}
#endif /* REALTEK_FEATURES */
...
#ifdef REALTEK_FEATURES
	if (wpa_drv_driver_cmd_set_aek(wpa_s, sta))
		wpa_printf(MSG_INFO, "mesh: Failed to set AEK");
#endif /* REALTEK_FEATURES */
}
